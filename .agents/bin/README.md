...
| `validate` | Pre-push gate | Resolves the configured base ref (fetching `origin` when available, otherwise using the local branch), then runs Git whitespace checks for unstaged, staged, and base-to-HEAD content plus UTF-8 and YAML parsing for the repository docs and policy |
